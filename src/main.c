...
static const char* STR_OVER = "over";
static const char* STR_VOOR = "voor";
...
void time_to_text(int hours, int minutes)
{
	...
	if (minutes == 0) {
        ...
        strcat(s_data.bufferc, " uur");
        set_central_text();
	}
	else if (minutes == 30) {
        strcat(s_data.bufferc, STR_HALF);
        strcat(s_data.bufferc, " ");
        strcat(s_data.bufferc, NUMWORDS[hours]);
        set_central_text();
	}
	else if (minutes <= 15) {
        ...
        strcat(s_data.buffer2, " ");
        strcat(s_data.buffer2, STR_HALF);
        strcat(s_data.buffer3, NUMWORDS[hours]);
        set_three_text();
	}
	else if (minutes < 45) {
